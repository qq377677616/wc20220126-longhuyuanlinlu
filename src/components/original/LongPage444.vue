<template>
  <div class="default-popup pof">
    <div class="page page444 flex-cen">
      <div class="con">
        <div class="con-one flex-cen">
          <div class="left">
            <transition name="left-right">
              <div class="img" v-show="showHide"><img src="../../assets/images/original/page17_01.png" alt=""/></div>
            </transition>
            </div>
            <div class="right">
              <div class="img-box">
                <transition name="right-left">
                  <img v-show="showHide" src="../../assets/images/original/page17_02.png" alt=""/>
                </transition>
                <transition name="right-left">
                  <img v-show="showHide" src="../../assets/images/original/page17_03.png" alt=""/>
                </transition>
                <transition name="right-left">
                  <img v-show="showHide" src="../../assets/images/original/page17_04.png" alt=""/>
                </transition>
                </div>
            </div>
        </div>
        <!-- 左下角菜单栏部分 -->
        <transition name="bottom-top2">
          <MenuList v-show="isShowMenu" ref="menuList" @selectMenu="selectMenu"></MenuList>
        </transition>
      </div>
    </div>
  </div>
</template>
<script>
import { reactive, toRefs, ref, provide, inject, onMounted } from 'vue'
export default {
  name: 'indexName',
  setup(props, { emit }) {
    provide("isShowBackBtn", true)
    onMounted(() => {
      // methods.play()
    })
    //页面数据
    const data = reactive({
      sequenceList: { url: `../assets/images/sequence/page1/page1_0.png`, num: 16, sequenceIndex: "sequenceListIndex", initIndex: 1, speed: 110, loop: true, autoplay: true },//序列配置
      sequenceListIndex: 0,//序列索引
      showHide: false,//显示隐藏变量
      mySetTimeout1: null,//定时器1
      mySetTimeout2: null,//定时器2
      isShowMenu: false,//展示菜单栏
    })
    //事件方法
    const methods = reactive({
      //开始加载
      play() {
        data.mySetTimeout1 = setTimeout(() => { data.showHide = true }, 400)
        data.mySetTimeout2 = setTimeout(() => { data.isShowMenu = true }, 700)
      },
      //移除加载效果
      removeAnimate() {
        clearTimeout(data.mySetTimeout1)
        clearTimeout(data.mySetTimeout2)
        data.showHide = false
        data.isShowMenu = false
      },
      //左下角菜单点击回调
      selectMenu(e) {
        //console.log("e", e)
        if (e[0] == 'back') {
          // if (data.showTouchPage == 441) data.transitionName = 'bottom-top2'
          emit('openDefaultBack')
        } 
      },
      //监听序列索引
      getSequenceListIndex(e) {
        data.sequenceListIndex = e
      }
    })
    return {
      ...toRefs(data),
      ...toRefs(methods),
    }
  }
}
</script>

<style scoped>
  .left,.img-box img:nth-child(1){transition-delay: .2s;}
  .img-box{position: relative;}
  .img-box img:nth-child(2),.img-box img:nth-child(3){position: absolute;left:0;top:calc(50% - 373.5px);z-index: 2;}
  .img-box img:nth-child(3){top:calc(50% - 265.5px);z-index: 3;}
  .img-box img:nth-child(1){transition-delay: .5s;}
  .img-box img:nth-child(2){transition-delay: .35s;}
</style>